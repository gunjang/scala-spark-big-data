...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    val firstField = StructField(columnNames.head, StringType, false)
    val tailFields = columnNames.tail.map( f => StructField(f, DoubleType, false) )
    StructType(firstField :: tailFields)
  }
  ...
  def row(line: List[String]): Row =
    Row.merge( Row(line.head), Row.fromSeq(line.tail.map(f => f.toDouble)) )

//  Row(line.head :: line.tail.map(f => f.toDouble))
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val primaryNeedsPfx = List("t01", "t03", "t11", "t1801", "t1803")
    val workingPfx = List("t05", "t1805")
    val otherPfx = List("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18")

    val primaryNeedsCols = columnNames.filter(c => primaryNeedsPfx.exists(pfx => c.startsWith(pfx))).map(c => col(c))
    val workingCols = columnNames.filter(c => workingPfx.exists(pfx => c.startsWith(pfx))).map(c => col(c))
    val otherCols = columnNames.filter(c => otherPfx.exists(pfx => c.startsWith(pfx)) &&
      !primaryNeedsPfx.exists(pp => c.startsWith(pp)) &&
      !workingPfx.exists(pp => c.startsWith(pp))
    ).map(c => col(c))
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed.groupByKey(d => (d.working, d.sex, d.age))
      ...
      .map { case(k, a1, a2, a3) => TimeUsageRow(k._1, k._2, k._3, a1, a2, a3) }
